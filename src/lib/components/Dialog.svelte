<script>
  ...
  import { Button } from '@oneezy/ui'
	import clickOutside from '@oneezy/ui/actions/useClickOutside'
...
{:else if type == 'nav'}
  ...
  {#if isOpen}
    <Button on:click={close} button="icon" icon="close" size="28" box="0 0 20 20" class="absolute left-0" style="position: absolute; left: -8px; width: auto; padding: 0; top: -14px; margin: 0;">Close</Button>
  {:else}
    <Button on:click={show} button="icon" icon="menu" size="28" box="0 0 20 20" class="absolute left-0" style="position: absolute; left: -8px; width: auto; padding: 0; top: -14px; margin: 0;">Menu</Button>
  {/if}
